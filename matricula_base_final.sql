...
DROP TABLESPACE LAB1 INCLUDING CONTENTS AND DATAFILES;
DROP TABLESPACE LAB1_TEMP INCLUDING CONTENTS AND DATAFILES;
DROP USER LAB1 CASCADE;
...
insert into CARRERA(ID, CODIGO, NOMBRE)  values(2, 'ADM', 'Administracion de Empresas');
insert into CARRERA(ID, CODIGO, NOMBRE)  values(3, 'MAT', 'Matematicas');
insert into CARRERA(ID, CODIGO, NOMBRE)  values(4, 'LIX', 'Ingles');
insert into CARRERA(ID, CODIGO, NOMBRE)  values(5, 'EDU', 'Educacion');
...
--INSERTAR
CREATE OR REPLACE PROCEDURE INSERTAR_PROFESOR(CEDULA IN PROFESOR.CEDULA%TYPE,NOMBRE IN PROFESOR.NOMBRE%TYPE,TELEFONO IN PROFESOR.TELEFONO%TYPE,EMAIL IN PROFESOR.EMAIL%TYPE, CURSO IN PROFESOR.CURSO%TYPE)

AS

BEGIN

INSERT INTO PROFESOR VALUES(SEC_PROFESOR.nextval, CEDULA, NOMBRE, TELEFONO, EMAIL, CURSO);
COMMIT;
...
--MODIFICAR
CREATE OR REPLACE PROCEDURE MODIFICAR_PROFESOR(P_CEDULA IN PROFESOR.CEDULA%TYPE,P_NOMBRE IN PROFESOR.NOMBRE%TYPE,P_TELEFONO IN PROFESOR.TELEFONO%TYPE,P_EMAIL IN PROFESOR.EMAIL%TYPE, P_CURSO IN PROFESOR.CURSO%TYPE)

AS

BEGIN

UPDATE PROFESOR SET NOMBRE = P_NOMBRE, TELEFONO = P_TELEFONO, EMAIL = P_EMAIL, CURSO = P_CURSO WHERE CEDULA = P_CEDULA;
COMMIT;
...
SELECT CEDULA, NOMBRE, TELEFONO, EMAIL, CURSO FROM PROFESOR WHERE CEDULA = P_CEDULA; 
...
SELECT CEDULA, NOMBRE, TELEFONO, EMAIL, CURSO FROM PROFESOR; 
...
UPDATE PROFESOR SET CURSO = NULL WHERE CURSO = P_CODIGO;
